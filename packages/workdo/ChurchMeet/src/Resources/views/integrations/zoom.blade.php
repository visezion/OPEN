...
@section('page-breadcrumb', __('Zoom Integration'))
  ...
  <div class="col-lg-6">
    <div class="card shadow-sm mb-3">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <div>
          <a href="{{ route('churchmeet.zoom.test') }}" class="btn btn-sm btn-outline-secondary">{{ __('Test Connection') }}</a>
          <a href="{{ route('churchmeet.zoom.sync') }}" class="btn btn-sm btn-outline-primary">{{ __('Sync Now') }}</a>
        </div>
      </div>
      <div class="card-body">
        ...
        <form method="POST" action="{{ route('churchmeet.zoom.save') }}">@csrf
          <div class="mb-3">
        ...
        <div class="list-group">
          @forelse($recentParticipants as $p)
            <div class="list-group-item">
              ...
              <small class="text-muted">{{ __('Join') }}: {{ optional($p->join_time)->toDateTimeString() }} Ã¢â‚¬Â¢ {{ __('Duration') }}: {{ $p->duration }}s</small>
            </div>
          @empty
