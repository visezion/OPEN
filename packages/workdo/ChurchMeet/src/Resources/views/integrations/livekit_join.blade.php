...
<div class="row livekit-room">
    <div class="col-lg-8">
        <div class="card mb-4">
            <div class="card-header bg-white d-flex justify-content-between align-items-center">
                <div>
                    <h5 class="mb-1">{{ $attendanceEvent->event->title ?? __('LiveKit Room') }}</h5>
                    <small class="text-muted">{{ __('Room') }}: {{ $livekitRoomName }}</small>
                </div>
                <div class="text-end">
                    <div class="fw-semibold"><span id="connection-dot" class="status-dot"></span> <span id="connection-status">{{ __('Not connected') }}</span></div>
                    <small class="text-muted">{{ $participantName }}</small>
                </div>
            </div>
            <div class="card-body">
                <div class="stage">
                    <div class="participant-grid" id="participant-grid">
                        <div class="participant-card" data-participant="placeholder">
                            <div class="participant-header">
                                <strong>{{ __('Waiting for room connection') }}</strong>
                            </div>
                ...
                <div class="d-flex flex-wrap gap-2 mt-3">
                    <button type="button" class="btn btn-primary" id="join-room">{{ __('Reconnect LiveKit Room') }}</button>
                    <button type="button" class="btn btn-outline-dark" id="toggle-fullscreen">{{ __('Full Screen') }}</button>
                    <button type="button" class="btn btn-outline-secondary" id="toggle-mic" disabled>{{ __('Mute Mic') }}</button>
                    <button type="button" class="btn btn-outline-secondary" id="toggle-camera" disabled>{{ __('Stop Camera') }}</button>
                    <button type="button" class="btn btn-outline-danger" id="leave-room" disabled>{{ __('Leave Room') }}</button>
                </div>
                <small class="text-muted d-block mt-2">{{ __('ChurchMeet will mark your presence when the room connects and update it again when you leave.') }}</small>
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-4">
        <div class="card mb-4">
            <div class="card-header bg-white"><h6 class="mb-0">{{ __('Room Details') }}</h6></div>
            <div class="card-body">
                <div class="d-flex justify-content-between mb-2"><span class="text-muted">{{ __('Platform') }}</span><strong>{{ __('LiveKit') }}</strong></div>
                <div class="d-flex justify-content-between mb-2"><span class="text-muted">{{ __('Room Name') }}</span><strong>{{ $livekitRoomName }}</strong></div>
                <div class="d-flex justify-content-between mb-2"><span class="text-muted">{{ __('Server URL') }}</span><strong>{{ $livekitServerUrl }}</strong></div>
                <div class="d-flex justify-content-between"><span class="text-muted">{{ __('Attendance Mode') }}</span><strong>{{ ucfirst($attendanceEvent->mode ?: 'online') }}</strong></div>
            </div>
        </div>

        <div class="card">
            <div class="card-header bg-white"><h6 class="mb-0">{{ __('Usage Notes') }}</h6></div>
            <div class="card-body">
                <p class="text-muted mb-2">{{ __('LiveKit rooms are browser-based. Members can join directly from ChurchMeet without leaving OPEN.') }}</p>
                <p class="text-muted mb-0">{{ __('If camera or microphone permissions are blocked, reconnect after granting access in your browser settings.') }}</p>
            </div>
        </div>
    </div>
</div>
