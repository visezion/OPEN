...
@section('page-title', __('Meeting Room'))

@section('page-breadcrumb')
    {{ __('ChurchMeet') }},{{ __('Meeting Room') }}
@endsection
...
@push('css')
<link rel="stylesheet" href="{{ asset('packages/workdo/ChurchMeet/src/Resources/assets/css/livekit-room.css') }}">
@endpush

@section('content')
<div class="livekit-room" data-brand-primary="{{ $brandPrimary }}" data-brand-rgb="{{ $brandRgbString }}">
    <div class="livekit-room-shell">
        <div class="meeting-topbar">
            <div class="meeting-brand">
                ...
                <div>
                    <div class="meeting-kicker">{{ __('ChurchMeet Live Session') }}</div>
                    <h2 class="meeting-title">{{ $meetingTitle }}</h2>
                    <p class="meeting-subtitle">{{ __('Join, chat, and watch the room from one screen.') }}</p>
                </div>
            </div>
            ...
            <div class="meeting-status-cluster">
                <div class="meeting-pill">
                    ...
                    <span id="connection-status">{{ __('Not connected') }}</span>
                </div>

                <div class="meeting-pill meeting-pill-live">
                    <span class="live-dot"></span>
                    <strong>{{ __('Live') }}</strong>
                    <span id="joined-minutes">{{ __('0 min') }}</span>
                </div>

                @auth
                    <a href="{{ route('churchmeet.events.show', optional($attendanceEvent->event)->public_view_key ?? $attendanceEvent->event_id) }}" class="meeting-pill text-decoration-none">
                        <i class="ti ti-external-link"></i>
                        <span>{{ __('Open Event') }}</span>
                    </a>
                @endauth

                <button type="button" class="meeting-pill meeting-copy-trigger" data-copy-text="{{ $meetingShareUrl }}" data-copy-default="{{ __('Copy Invite') }}" data-copy-success="{{ __('Copied') }}">
                    <i class="ti ti-link"></i>
                    <span>{{ __('Copy Invite') }}</span>
                </button>

                <div class="meeting-avatar">{{ $participantBadge ?: 'WD' }}</div>
            </div>
        </div>
        ...
        @if($requiresGuestName)
            <section class="meeting-gate-panel">
                <div class="meeting-gate-card">
                    ...
                    <h3>{{ __('Enter your display name') }}</h3>
                    <p>{{ __('Sign in with your WorkDo account or continue as a guest. Guest access only needs the name other participants should see.') }}</p>

                    <form action="{{ url()->current() }}" method="GET" class="meeting-gate-form">
                        <div>
                            <label for="guest_name" class="meeting-gate-label">{{ __('Display Name') }}</label>
                            <input
                                type="text"
                                id="guest_name"
                                name="guest_name"
                        ...
                        <div class="meeting-gate-actions">
                            <a href="{{ $meetingLoginUrl }}" class="meeting-gate-button is-secondary">
                                <i class="ti ti-login"></i>
                                <span>{{ __('Login') }}</span>
                            </a>
                            <button type="submit" class="meeting-gate-button">
                                <i class="ti ti-door-enter"></i>
                                <span>{{ __('Join as Guest') }}</span>
                            </button>
                            <span class="meeting-gate-note">{{ __('Your name will be reused for this browser session until you change it.') }}</span>
                        </div>
                    </form>
                </div>
            </section>
                ...
                <div class="stage-shell">
                    <div class="stage-topbar">
                        <div class="stage-indicator">
                            ...
                            <span id="stage-connection-status">{{ __('Waiting for connection') }}</span>
                        </div>

                        <div class="stage-helper">{{ __('Room controls stay pinned below.') }}</div>
                    </div>
                        ...
                        <div class="participant-grid" id="participant-grid">
                            <div class="participant-card" data-participant="placeholder">
                                <div class="participant-media">
                                    ...
                                    <div class="participant-placeholder">{{ __('Connect to load your local stream and any remote participants.') }}</div>
                                </div>
                                <div class="participant-footer">
                                    <span class="participant-name">{{ __('Waiting for room') }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="stage-footer">
                        ...
                        <div class="control-layout">
                            ...
                            <button type="button" class="meeting-control is-primary is-stack" id="join-room">
                                ...
                                <span class="control-label">{{ __('Reconnect') }}</span>
                            </button>
                            </div>

                            <div class="control-dock control-dock-center">
                            <button type="button" class="meeting-control is-stack" id="toggle-mic" data-static-label="{{ __('Audio') }}" aria-label="{{ __('Mute Mic') }}" title="{{ __('Mute Mic') }}" disabled>
                                <i class="control-icon ti ti-microphone"></i>
                                <span class="control-label">{{ __('Audio') }}</span>
                            </button>

                            <button type="button" class="meeting-control is-stack" id="toggle-camera" data-static-label="{{ __('Video') }}" aria-label="{{ __('Stop Camera') }}" title="{{ __('Stop Camera') }}" disabled>
                                <i class="control-icon ti ti-video"></i>
                                <span class="control-label">{{ __('Video') }}</span>
                            </button>

                            <button type="button" class="meeting-control is-stack" id="toggle-sidebar" aria-label="{{ __('Hide Participants') }}" title="{{ __('Hide Participants') }}">
                                <i class="control-icon ti ti-users"></i>
                                <span class="control-label">{{ __('Participants') }}</span>
                            </button>

                            <button type="button" class="meeting-control is-stack" id="toggle-hand-raise" data-static-label="{{ __('Hand') }}" aria-label="{{ __('Raise Hand') }}" title="{{ __('Raise Hand') }}" disabled>
                                <i class="control-icon ti ti-hand-click"></i>
                                <span class="control-label">{{ __('Hand') }}</span>
                            </button>

                            <button type="button" class="meeting-control is-stack" id="toggle-reactions" data-static-label="{{ __('React') }}" aria-label="{{ __('Open Reactions') }}" title="{{ __('Open Reactions') }}" disabled>
                                <i class="control-icon ti ti-mood-smile"></i>
                                <span class="control-label">{{ __('React') }}</span>
                            </button>

                            <button type="button" class="meeting-control is-stack" id="toggle-fullscreen" aria-label="{{ __('Full Screen') }}" title="{{ __('Full Screen') }}">
                                <i class="control-icon ti ti-maximize"></i>
                                <span class="control-label">{{ __('Full Screen') }}</span>
                            </button>

                            <button type="button" class="meeting-control is-stack" id="toggle-screen-audio-share" data-static-label="{{ __('Share') }}" aria-label="{{ __('Share Screen + Audio') }}" title="{{ __('Share Screen + Audio') }}" disabled>
                                <i class="control-icon ti ti-screen-share"></i>
                                <span class="control-label">{{ __('Share') }}</span>
                            </button>
                            </div>

                            <div class="control-dock control-dock-right">
                            <button type="button" class="meeting-control is-danger is-stack" id="leave-room" data-static-label="{{ __('Exit') }}" aria-label="{{ __('Exit') }}" title="{{ __('Exit') }}" disabled>
                                <i class="control-icon ti ti-logout"></i>
                                <span class="control-label">{{ __('Exit') }}</span>
                            </button>
                            </div>
                        </div>

                        <div class="reaction-picker" id="reaction-picker" hidden>
                            <button type="button" class="reaction-picker-button" data-reaction="👍" aria-label="{{ __('Thumbs up') }}">👍</button>
                            <button type="button" class="reaction-picker-button" data-reaction="👏" aria-label="{{ __('Clap') }}">👏</button>
                            <button type="button" class="reaction-picker-button" data-reaction="❤️" aria-label="{{ __('Love') }}">❤️</button>
                            <button type="button" class="reaction-picker-button" data-reaction="😂" aria-label="{{ __('Laugh') }}">😂</button>
                            <button type="button" class="reaction-picker-button" data-reaction="🎉" aria-label="{{ __('Celebrate') }}">🎉</button>
                            <button type="button" class="reaction-picker-button" data-reaction="🙏" aria-label="{{ __('Pray') }}">🙏</button>
                        </div>
                    </div>
                </div>
            </section>

            <button type="button" class="sidebar-popup-backdrop" id="sidebar-popup-backdrop" aria-label="{{ __('Close sidebar popup') }}"></button>

            <aside class="sidebar-panel">
                <div class="sidebar-card sidebar-card-main">
                    <div class="sidebar-header">
                        <div>
                            <div class="sidebar-eyebrow">{{ __('Live Sidebar') }}</div>
                            <h4>{{ __('Chat & People') }}</h4>
                        </div>
                    ...
                    <div class="sidebar-view" id="chat-view">
                        <div class="chat-stream" id="chat-stream">
                            <div class="chat-empty" id="chat-empty-state">
                                <div>
                                    ...
                                    <div class="fw-semibold">{{ __('No messages yet') }}</div>
                                    <div>{{ __('Chat clears when the meeting ends.') }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <form class="chat-compose" id="chat-form">
                            <div class="compose-row">
                                ...
                                <span class="sidebar-note">{{ __('Messages disappear after the room closes.') }}</span>
                            </div>

                            <div class="compose-target-row">
                                <select id="chat-target" class="compose-target-select" aria-label="{{ __('Choose message recipient') }}">
                            ...
                            <div class="compose-input-wrap">
                                <input type="text" id="chat-input" class="compose-input-control" maxlength="400" placeholder="{{ __('Message everyone') }}">
                        ...
                        <div class="participants-list" id="participants-list">
                            <div class="chat-empty">
                                <div>
                                    ...
                                    <div class="fw-semibold">{{ __('Waiting for participants') }}</div>
                                    <div>{{ __('People and their mic/camera status will appear here.') }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
const serverUrl = @json($livekitServerUrl);
const token = @json($livekitToken);
const participantName = @json($participantName);
const participantAvatarUrl = @json($participantAvatarUrl ?? null);
const presenceUrl = @json(route('churchmeet.meetings.presence', $attendanceEvent->public_join_key));
