...
<head>
    ...
    <title>@yield('page-title') | {{ $appTitle }}</title>
    ...
    <style>
        :root {
            --churchmeet-public-primary: {{ $brandPrimary }};
            --churchmeet-public-primary-rgb: {{ $brandRgbString }};
            --churchmeet-public-ink: #102840;
            --churchmeet-public-muted: #61748d;
            --churchmeet-public-border: rgba(16, 40, 64, 0.12);
            --churchmeet-public-soft: rgba({{ $brandRgbString }}, 0.08);
        }
        ...
        body.churchmeet-public-layout {
            ...
            min-height: 100vh;
            background:
                radial-gradient(circle at top left, rgba({{ $brandRgbString }}, 0.16), transparent 30%),
                radial-gradient(circle at right 12%, rgba({{ $brandRgbString }}, 0.10), transparent 26%),
        ...
        .churchmeet-public-header {
            ...
            background: linear-gradient(180deg, rgba({{ $brandRgbString }}, 0.07), rgba(255, 255, 255, 0.86));
        }
        ...
        .churchmeet-public-pill.is-primary {
            ...
            background: rgba({{ $brandRgbString }}, 0.10);
            border-color: rgba({{ $brandRgbString }}, 0.18);
        }
        ...
        <div class="churchmeet-public-shell">
            <header class="churchmeet-public-header">
                <div class="churchmeet-public-brand">
                    ...
                    <div>
                        <p class="churchmeet-public-kicker">{{ __('ChurchMeet Public Access') }}</p>
                        <h1 class="churchmeet-public-title">@yield('page-title')</h1>
                        <p class="churchmeet-public-subtitle">{{ __('Join the meeting securely from any device without signing in.') }}</p>
                    </div>
                </div>
                <div class="churchmeet-public-badges">
                    <span class="churchmeet-public-pill is-primary"><i class="ti ti-shield-check"></i> {{ __('Secure Join') }}</span>
                    <span class="churchmeet-public-pill"><i class="ti ti-device-laptop"></i> {{ __('Works in Browser') }}</span>
                </div>
            </header>
