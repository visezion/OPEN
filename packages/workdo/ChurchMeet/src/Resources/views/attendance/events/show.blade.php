...
@section('page-action')
    @if($canJoinZoomMeeting)
        <a href="{{ route('churchmeet.zoom.meetings.join', $attendanceEvent->id) }}" class="btn btn-primary btn-sm">
                        ...
                        <div class="border-top pt-3 mt-2">
                            <div class="row">
                                <div class="col-md-4">
                                    ...
                                    <strong>{{ $event->lead->name ?? 'Ã¢â‚¬â€' }}</strong>
                                </div>
                                <div class="col-md-4">
                                    <p class="mb-1 text-muted small">Assistant / Co-Leader</p>
                                    <strong>{{ $event->assistant->name ?? 'Ã¢â‚¬â€' }}</strong>
                                </div>
                            </div>
                        </div>
                                        ...
                                        @if($attendanceEvent->meeting_id)
                                            <div class="small text-muted mt-1">
                                                ...
                                                @if($attendanceEvent->meeting_passcode)
                                                    Ã¢â‚¬Â¢ {{ __('Passcode') }}: {{ $attendanceEvent->meeting_passcode }}
                                                @endif
                                            </div>
                                        @endif
                                    </div>
                                    ...
                                    <tbody>
                                        @forelse($event->programs as $program)
                                            <tr>
                                                ...
                                                <td class="text-muted">{{ $program->note ?? 'Ã¢â‚¬â€' }}</td>
                                            </tr>
                                        @empty
                                            <tr>
                                    ...
                                    <div class="p-3 rounded {{ $comment->user_id === Auth::id() ? 'bg-primary text-white' : 'bg-white border' }}"
                                        ...
                                        <div class="small fw-bold mb-1">
                                            ...
                                            <span class="text-muted small">
                                                Ã¢â‚¬Â¢ {{ \Carbon\Carbon::parse($comment->commented_at)->diffForHumans() }}
                                            </span>
                                        </div>
